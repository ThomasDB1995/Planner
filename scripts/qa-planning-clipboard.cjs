...
(async () => {
  ...
  try {
    ...
    const paste = () => page.getByRole("button", { name: "Hier plakken", exact: true });
    await card().click();
    await page.getByPlaceholder("Bijvoorbeeld: werf voorbereiden").fill("QA Maaiwerk bijgewerkt");
    await card().focus();
    await page.keyboard.press("Control+c");
    await page.getByText("Gekopieerd:", { exact: true }).waitFor();
    ...
    await page.getByRole("button", { name: "Kopiëren", exact: true }).click();
    ...
    assert.equal(await page.getByText("Geknipt:", { exact: true }).count(), 0);
    await card().click();
    ...
    await card().click();
    await page.getByRole("button", { name: "Knippen", exact: true }).click();
    await page.getByText("Geknipt:", { exact: true }).waitFor();
